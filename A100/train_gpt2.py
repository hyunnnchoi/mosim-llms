...
def main():
    ...
    while step < max_steps:
        ...
        if not is_primary and step > 0 and should_stop(args.barrier_dir, partner_id, rank):
            if rank == 0:
                print(f"  [GPT2] interferer stopping at step {step} (primary is done)")
            break
        ...
        if rank == 0 and (step % 10 == 0 or (is_primary and step == args.total_steps - 1)):
            ...
            print(f"  [GPT2/{role_tag}] step {step:>4d}/{args.total_steps if is_primary else '?'} | "
    ...
    if is_primary:
        ...
        if rank == 0:
            print(f"[GPT2] Interferer finished after {step} steps.")
