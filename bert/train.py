"""BERT training script with DDP support and Chakra tracing."""
...
def evaluate(model: nn.Module, val_loader, config: BERTConfig):
    """Evaluate the model."""
    model.eval()
    total_loss = 0
    step = 0
...
def train(config: BERTConfig):
    ...
    if is_main_process():
        print(f"Training BERT on SQuAD dataset")
...
def main():
    """Entry point."""
    parser = argparse.ArgumentParser(description="BERT Pretraining on SQuAD")
    ...
    parser.add_argument("--trace-name", type=str, default="bert_trace", help="Trace name")
    
    # Paths
    parser.add_argument("--save-dir", type=str, default="./checkpoints/bert", help="Save directory")
